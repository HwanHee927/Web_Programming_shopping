...
<head>
    ...
    function addToCart() {
        ...
        if (confirm("상품이 장바구니에 등록되었습니다! 장바구니로 이동하시겠습니까?")) {
            // 장바구니로 이동
            window.location.href = "itemPayment.php"; // 팝업메시지를 띄움
        } else { return false; }
    }
    </script>
</head>
...
<body class="bg-gray-100">
    ...
    <main class="container mx-auto mt-8">
        <div class="bg-white shadow-md rounded-lg p-6">
            <?php
            // itemDetails.php에서 'bigImage' 값을 받아오기
            if (isset($_GET['bigImage'])) {
                $bigImage = $_GET['bigImage'];
            } else {
                // 기본 이미지가 필요하면 여기에 추가할 수 있습니다
                $bigImage = 'default_image'; // 예: 기본 이미지
            }
            ?>
        <div class="flex justify-center mb-6">
            <!-- 동적으로 이미지를 변경 -->
            <img src="../image/<?= $bigImage ?>.png" alt="Product Image" class="w-1/2 rounded-lg">
        </div>
        ...
        <button class="bg-white px-6 py-2 rounded shadow" onclick="window.history.back();"> 돌아가기 </button>
            <form action="add_to_cart.php" method="post">
                <button class="bg-orange-500 text-white px-6 py-2 rounded shadow" onclick="addToCart()">장바구니 담기</button>
            </form>
        </div>
        <br>
    </main>
